...
const CGFloat kJSQMessagesEditCollectionOverlayViewIconHeight = 27.0;
...
- (void)setIsActive:(BOOL)isActive
{
    ...
    if (isActive) {
        self.leftIconView.image = [self.delegate editingActiveImage];
        self.leftIconView.tintColor = [self.delegate editingActiveColor];
    } else {
        self.leftIconView.image = [self.delegate editingInactiveImage];
        self.leftIconView.tintColor = [self.delegate editingInactiveColor];
    }

    self.rightIconView.image = self.leftIconView.image;
    self.rightIconView.tintColor = self.leftIconView.tintColor;
}
